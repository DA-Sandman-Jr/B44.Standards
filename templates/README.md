...
| `format.yml` | `.github/workflows/` | Format-only gate: `dotnet format --verify-no-changes` over the root solution, accepting `.sln` or `.slnx`. Fails with a named error when the repository has no root solution, rather than reporting green having checked nothing. A repository that also wants the Core build and test run should call `reusable-dotnet-ci.yml` instead of adding this file |
