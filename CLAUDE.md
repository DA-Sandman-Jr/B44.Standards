...
- Hosted GitHub Actions are landing and integration gates, not the normal development feedback loop. Prefer local build, test, formatting, architecture, and engine or runtime validation throughout implementation, then use hosted validation on the completed landing candidate and fix failures on that same PR.
