...
- When approved work on a non-default GitHub branch is meant to land, ensure a pull request exists and merge through that pull request rather than direct-merging the branch into its base. The PR is the merge and branch-cleanup mechanism, not an extra review gate: substantive review may happen elsewhere, and any explicit human-approval requirement still controls whether merge is allowed. A PR may be created only at closure if that best fits the workflow. After landing, verify both the merged PR and removal of the head branch when the repository is configured for automatic head-branch deletion; if repository rules or settings prevent deletion, report the residual branch explicitly. Never merge rejected, abandoned, or unapproved work merely to trigger cleanup.
